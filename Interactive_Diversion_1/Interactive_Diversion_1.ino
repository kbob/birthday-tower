...
#define MAX_V ((uint32_t)400 * 256)  // steps/tick * 256

#define MAX_SPARKBURST 30
#define MIN_SPARKBURST 5
#define SPARKBURST_SLOWEST 200LL
#define SPARKBURST_FASTEST 40LL

// SPI: data = D11, clock = D13.
// Attach button between pins D7 and ground.

const int BUTTON_PIN = 6;
// const int PIXEL_COUNT = 32 * 5;
const int PIXEL_COUNT = 200;
const int UPDATE_MS = 7;
const uint32_t IDLE_TIMEOUT = 90000;    // milliseconds
const int ANALOG_PINS[] = {A0, A1, A2, A3};
const size_t ANALOG_PIN_COUNT = sizeof ANALOG_PINS / sizeof ANALOG_PINS[0];
...
// LPD8806 strip;
Adafruit_NeoPixel strip(PIXEL_COUNT, 1, NEO_GRB);
...
void setup(void)
{
    ...
    for (size_t i = 0; i < ANALOG_PIN_COUNT; i++)
      seed = seed << 3 ^ analogRead(ANALOG_PINS[i]);
    randomSeed(seed);
    ...
    idle_timeout = millis() + IDLE_TIMEOUT / 5;
}
...
void loop(void)
{
    ...
    if (millis() - last_update_time >= UPDATE_MS) {
      last_update_time += UPDATE_MS;
      callback();
    }
}
